...
### Fluxo principal:
1. O usuário seleciona a turma desejada e solicita a realização de matrícula. (FE-1-A; FE-1-B)
2. O sistema apresenta um campo de busca para selecionar o beneficiário. (FA-2-A)
...
Este fluxo inicia no passo 2 do fluxo principal. Se não houver beneficiários cadastrados ou aptos para matrícula no sistema, o sistema exibe um aviso informando que a lista de beneficiários está vazia. O caso de uso é suspenso.
...
Este fluxo inicia no passo 1 do fluxo principal. Se o usuário logado não possuir papel que permita efetuar matrículas (ex: perfil não autorizado), o sistema bloqueia o acesso e exibe erro de permissão. O caso de uso é encerrado.

#### FE-4-A — Item Inexistente (Beneficiário)
Este fluxo inicia no passo 4 do fluxo principal. Se o beneficiário selecionado não for encontrado na base de dados (deletado simultaneamente), a matrícula é cancelada exibindo erro. O fluxo retorna ao passo 2 do fluxo principal.
