...
1. O usuário acessa o menu de beneficiários [[FE-1-A](#fe-1-a-permissao-insuficiente)] e clica em 'Cadastrar Beneficiário'.
...
4. O sistema valida o formato dos dados preenchidos e a unicidade do CPF (se fornecido). [[FE-4-A](#fe-4-a-dados-obrigatorios-ausentes), [FE-4-B](#fe-4-b-formato-de-cpf-invalido), [FE-4-C](#fe-4-c-cpf-de-beneficiario-ja-cadastrado), [FE-4-D](#fe-4-d-dados-invalidos), [FE-4-E](#fe-4-e-e-mail-ja-cadastrado)]
5. O sistema armazena o cadastro com o status de beneficiário ativo e gera um identificador único (ID). [[FE-5-A](#fe-5-a-falha-de-persistencia)]
...
No passo 1, se o perfil do usuário logado não for autorizado a gerenciar beneficiários, o sistema bloqueia o acesso à tela e retorna mensagem de erro de permissão insuficiente.
...
No passo 4, se o formato do e-mail do beneficiário ou do responsável estiver inválido, o sistema impede a gravação, exibe erros específicos de formatação e solicita a correção.

#### FE-4-E - E-mail já Cadastrado
No passo 4, se o e-mail informado já estiver cadastrado para outro beneficiário, o sistema bloqueia o registro, exibe mensagem de duplicidade de e-mail e solicita a correção.

#### FE-5-A - Falha de Persistência
No passo 5, se ocorrer uma falha ao persistir os dados do beneficiário no banco de dados, o sistema impede a operação, exibe mensagem de erro de conexão com o banco e mantém as informações no formulário.
