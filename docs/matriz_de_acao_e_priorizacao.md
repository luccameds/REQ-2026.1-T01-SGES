...
| **Cadastro Sociodemográfico**       | **US07** | Como gestor ou instrutor, quero cadastrar beneficiários (alunos) vinculados a famílias para organizar melhor o acompanhamento social. | • **CA07-01:** Beneficiário (aluno) criado com sucesso no sistema.<br>• **CA07-02:** Obrigatória a associação do beneficiário (aluno) a um núcleo familiar válido.                                                                                                 | RF07 - Cadastrar beneficiário (aluno)              | RNF01, RNF08     |
| **Cadastro Sociodemográfico**       | **US08** | Como gestor ou instrutor, quero editar os dados dos beneficiários (alunos) para manter as informações atualizadas ao longo do tempo.  | • **CA08-01:** Alterações salvas com sucesso no banco de dados.<br>• **CA08-02:** Dados atualizados exibidos corretamente na interface de usuário.                                                                                                 | RF08 - Editar dados do beneficiário (aluno)        | RNF01, RNF08     |
...
| **Frequência e Engajamento**        | **US10** | Como gestor ou instrutor, quero matricular beneficiários (alunos) em turmas para controlar a participação nas atividades.             | • **CA10-01:** Vínculo de matrícula criado com sucesso.<br>• **CA10-02:** Sistema bloqueia novas matrículas caso o limite de vagas seja excedido.                                                                                                  | RF10 - Matricular beneficiário (aluno)             | RNF09            |
| **Frequência e Engajamento**        | **US11** | Como instrutor, quero registrar presença em lote para agilizar o controle de frequência dos beneficiários (alunos).                    | • **CA11-01:** Registro de presença/falta salvo simultaneamente para todos os beneficiários (alunos) da lista.<br>• **CA11-02:** Lançamento permitido apenas na data em que a aula ocorre.                                                                 | RF11 - Registrar presença em lote          | RNF04, RNF05     |
...
| **Monitoramento de Evasão**         | **US14** | Como sistema, quero emitir alertas de evasão automaticamente para apoiar a identificação de beneficiários (alunos) em risco.           | • **CA14-01:** Identificação automática de 3 faltas consecutivas.<br>• **CA14-02:** Identificação automática de 5 faltas alternadas dentro do período.<br>• **CA14-03:** Alerta visual gerado e exibido nos painéis do sistema.                    | RF14 - Emitir alerta de evasão             | RNF06            |
| **Monitoramento de Evasão**         | **US15** | Como usuário, quero consultar o histórico do beneficiário (aluno) para acompanhar sua participação e evolução.                       | • **CA15-01:** Exibição clara e consolidada de todas as presenças.<br>• **CA15-02:** Histórico de alertas de risco emitidos.<br>• **CA15-03:** Matrículas anteriores e atuais listadas em ordem cronológica.                                       | RF15 - Consultar histórico do beneficiário (aluno) | RNF01, RNF02     |
